...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, L_SQUIR_IMG, R_SQUIR_IMG, GRASSIMAGES, TOMBIMAGES
    ...
    TOMBIMAGES = []
    for i in range(1, 3):
        TOMBIMAGES.append(pygame.image.load('tomb%s.png' % i))
...
def runGame():
    ...
    while True: # main game loop
        ...
        nbboss = 0
        for sq in squirrelObjs:
            if sq['isboss']:
                nbboss += 1
            
        while nbboss < NUMSQUIRRELSBOSS:
            squirrelObjs.append(makeNewSquirrel(camerax, cameray, True))
            nbboss += 1

        while len(squirrelObjs) < NUMSQUIRRELS:
            squirrelObjs.append(makeNewSquirrel(camerax, cameray, False))
        ...
        for gObj in grassObjs:
            gRect = pygame.Rect( (gObj['x'] - camerax,
                                  ...
                                  gObj['width'],
                                  gObj['height']) )
            DISPLAYSURF.blit(gObj['grassImage'], gRect)
...
def drawHealthMeter(currentHealth):
    for i in range(currentHealth): # draw red health bars
        pygame.draw.rect(DISPLAYSURF, RED,   (15, 3 + (3 * MAXHEALTH) - i * 3, 20, 10))
    for i in range(MAXHEALTH): # draw the white outlines
        pygame.draw.rect(DISPLAYSURF, WHITE, (15, 3 + (3 * MAXHEALTH) - i * 3, 20, 10), 1)
...
def makeNewGrass(camerax, cameray):
    ...
    if random.randint(0,55) == 0:
        gr['grassImage'] = TOMBIMAGES[random.randint(0, len(TOMBIMAGES) - 1)]
    else:
        gr['grassImage'] = GRASSIMAGES[random.randint(0, len(GRASSIMAGES) - 1)]
    gr['width']  = gr['grassImage'].get_width()
    gr['height'] = gr['grassImage'].get_height()
